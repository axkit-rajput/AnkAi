...
<script>
  ...
  const PanelContent = ({onClose}) => {
    return (
      <>
        {!collapsed ? <div className='flex flex-col h-full bg-[var(--ankai-sidebar)]'>
            ...
            {canPreview &&
              <div className='flex items-center gap-1 bg-white/[0.04] border border-[var(--ankai-border-soft)] p-1 rounded-lg'>
                <button
                  ...
                  ${tab === "code" ? "bg-[var(--ankai-accent)] text-white" : "text-slate-500 hover:text-slate-200"}`}
                >
                  <Code2 size={11} /> Code
                </button>
                <button
                  ...
                  className={`flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-medium rounded-md transition-colors duration-150
                  ${tab === "preview" ? "bg-[var(--ankai-accent)] text-white" : "text-slate-500 hover:text-slate-200"}`}
                >
                  <Eye size={11} /> Preview
                </button>
              </div>}
          ...
          {tab === "code" && <div className='flex h-auto border-b border-[var(--ankai-border-soft)] overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden shrink-0'>
            {
              artifacts[0]?.files?.map((f, index) => (
                ...
                >
                  {f?.name}
                  {activeFile === index && <div className='absolute bottom-0 left-0 right-0 h-[2px] bg-[var(--ankai-accent)] rounded-t-full' />}
    ...
    <>
      ...
      <AnimatePresence>
        {mobileOpen && <>
          ...
          <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ duration: 0.25, ease: "easeInOut" }} className="lg:hidden fixed inset-y-0 right-0 z-50 w-[88vw] max-w-[420px] border-l border-[var(--ankai-border-soft)] overflow-hidden">
